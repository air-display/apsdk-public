...
bool Java_com_medialab_airplay_AirPlayServer_start(JNIEnv *env, jobject thiz) {

  // Launch the mdnsd daemon process
  jclass thisClass = env->GetObjectClass(thiz);
  jmethodID acquireMdnsd = env->GetMethodID(thisClass, "acquireMdnsd", "()V");
  ...
  __android_log_write(ANDROID_LOG_DEBUG, LOG_TAG, info.c_str());
  return result;
}

void Java_com_medialab_airplay_AirPlayServer_stop(JNIEnv *env, jobject thiz) {

  // Stop the mdnsd daemon process
  jclass thisClass = env->GetObjectClass(thiz);
  jmethodID releaseMdnsd = env->GetMethodID(thisClass, "releaseMdnsd", "()V");
