...
namespace aps { namespace service { 
    ...
    void ap_video_stream_session::post_receive_packet_header()
    {
        ...
        asio::async_read(socket_, asio::buffer(&(packet_.header), packet_header_t::LENGTH),
            asio::bind_executor(
                strand_,
                std::bind(
                    &ap_video_stream_session::on_packet_header_received,
    ...
    void ap_video_stream_session::on_packet_header_received(const asio::error_code& e, std::size_t bytes_transferred)
    {
        if (!e) 
        {
    ...
    void ap_video_stream_session::post_receive_packet_payload()
    {
        ...
        asio::async_read(socket_, asio::buffer(packet_.payload.data(), packet_.header.payload_size),
            asio::bind_executor(
                strand_,
                std::bind(
                    &ap_video_stream_session::on_packet_payload_received,
                    shared_from_this(), // Keep the session alive
                    std::placeholders::_1,
                    std::placeholders::_2))
        );
    ...
    void ap_video_stream_session::on_packet_payload_received(const asio::error_code& e, std::size_t bytes_transferred)
    {
        if (!e)
        {
