...
namespace aps { namespace service { namespace details {
    ...
    public:
        ...
        void format_to_stream(asio::streambuf& buf) const
        {
            ...
            os << scheme_version << CHAR_BLANK << status_code << CHAR_BLANK << status_text << RN_LINE_BREAK;
            
            for (auto &kv : headers)
                os << kv.first << CHAR_COLON << CHAR_BLANK << kv.second << RN_LINE_BREAK;

            os << HEADER_CONTENT_LENGTH << CHAR_COLON << CHAR_BLANK << content_length << RN_LINE_BREAK;

            if (!content_type.empty())
                os << HEADER_CONTENT_TYPE << CHAR_COLON << CHAR_BLANK << content_type << RN_LINE_BREAK;

            os << RN_LINE_BREAK;

            std::copy(content.begin(), content.end(), std::ostream_iterator<uint8_t>(os));
        }
    };
