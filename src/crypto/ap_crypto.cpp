...
aps::server_key_chain::server_key_chain() {
  ...
  ed25519_create_keypair(ed_public_key_.data(), ed_private_key_.data(), seed);
}
...
aps::ap_crypto::ap_crypto()
    : fp_key_message_(164), client_aes_key_(16), client_aes_iv_(16),
      client_encrypted_aes_key_(64), client_curve_public_key_(32),
...
void aps::ap_crypto::init_pair_verify_aes() {
  ...
  AES_init_ctx_iv(&pair_verify_aes_ctr_ctx, sha512_aes_key.data(),
                  sha512_aes_iv.data());
}
...
void aps::ap_crypto::sign_pair_signature(std::vector<uint8_t> &sig) {
  ...
  std::copy(client_curve_public_key_.data(),
            client_curve_public_key_.data() + 32, std::back_inserter(message));
  ...
  AES_CTR_xcrypt_buffer(&pair_verify_aes_ctr_ctx, signature.data(),
                        signature.size());
...
bool aps::ap_crypto::verify_pair_signature(const uint8_t *p, uint64_t len) {
  ...
  AES_CTR_xcrypt_buffer(&pair_verify_aes_ctr_ctx, plain_signature.data(),
