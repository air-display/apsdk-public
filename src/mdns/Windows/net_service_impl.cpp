    ...
    : public net_service::implementation
{
    ...
    typedef void(DNSSD_API *DNSServiceRefDeallocate_t)(
        DNSServiceRef                       sdRef);
    ...
    typedef void(DNSSD_API *TXTRecordDeallocate_t)(
        TXTRecordRef                        *txtRecord);
...
public:
    net_service_impl(const std::string& type)
    {
        ...
        if (dnssd_lib_.Module)
        {
            ...
            dnssd_lib_.DNSServiceRefDeallocate = (DNSServiceRefDeallocate_t)GetProcAddress(dnssd_lib_.Module, "DNSServiceRefDeallocate");
            ...
            dnssd_lib_.TXTRecordGetBytesPtr = (TXTRecordGetBytesPtr_t)GetProcAddress(dnssd_lib_.Module, "TXTRecordGetBytesPtr");
            ...
            LOGE() << "Failed to load dnssd.dll. Please confirm the Bonjour Service has been installed successfully.";
    }
    ...
    virtual void add_txt_record(const std::string& k, const std::string& v) override
    {
        ...
        {
            auto error = dnssd_lib_.TXTRecordSetValue(&txt_records_,
    ...
    {
        if (dnssd_lib_.Module
            && dnssd_lib_.DNSServiceRegister
            && dnssd_lib_.TXTRecordGetLength
            && dnssd_lib_.TXTRecordGetBytesPtr)
        {
            auto error = dnssd_lib_.DNSServiceRegister(
                &dns_service_,
                0, 0,
                name.c_str(), type_.c_str(),
                0, 0,
                htons(port),
